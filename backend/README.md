...
The runtime image also includes Poppler, Tesseract and the Russian language pack. If a downloaded PDF has no extractable text, the files worker performs a bounded OCR pass (first 20 pages) and records `regex_ocr` provenance; unavailable or empty OCR remains `needs_review`.
